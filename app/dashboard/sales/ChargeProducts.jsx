...
export default function ChargeProducts(props) {
  ...
  const handleChangeProduct = (event) => {
    ...
    setSelectedProduct(idProducto); // <-- Cambiar aquí
  ...
  const handleAddButtonClick = () => {
    ...
    const subTotalPriceValue = parseFloat(
      (price - discount) / parseFloat(process.env.NEXT_PUBLIC_SACAR_IVA)
    ).toFixed(2);
    ...
    const totalWithtaxesValue = parseFloat(
      parseFloat(subTotalPriceValue) + parseFloat(taxes)
    ).toFixed(2);
    ...
    if (
      !isNaN(quantityValue) &&
      quantityValue > 0 &&
      !isNaN(subTotalPriceValue) &&
      subTotalPriceValue > 0
    ) {
      onAddProduct({
  ...
  return (
    <>
<div className="flex flex-col md:flex-row md:items-center border-solid border-2 border-sky-500 m-4 p-2 rounded shadow-lg shadow-gray-400 md:justify-between">
  {/* Seleccion de producto */}
  <div className="flex-col mt-3 md:mt-0 md:mr-3">
    <p className="mr-3">Producto</p>
    <select
      className="w-full md:w-64 md:h-8 text-black text-center rounded"
...
{/* Agregar producto */}
<div className="flex  mt-5 md:mt-0 md:mr-3 text-center justify-center p-3">
  <button
    className="md:w-64 text-white bg-gradient-to-r from-blue-500 via-blue-600 to-blue-700 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 shadow-lg shadow-blue-500/50 dark:shadow-lg dark:shadow-blue-800/80 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2"
